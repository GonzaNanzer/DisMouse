...
class DisMouse (tk.Frame):
    ...
    """    def nueva_ventana(self):
        if self.arduino.isOpen():
            #valor = str(self.combobox_Rojo.get())
            #self.arduino.write(valor.encode('ascii'))
            self.raiz2 = tk.Toplevel()
            #self.combobox_Rojo.current(1)
    ...
    def widgets(self):
        ...
        self.botonSetOrientacion.config(width=15)
               ...
               # (3.2.7    )*****---------------------Frame 3 - Botones Funciones ---------------------*****#
        ...
        self.SubFrame8 = tk.LabelFrame(self.Frame3, text='Configuracion boton orientacion')
